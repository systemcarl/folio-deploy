...
@test "sets status when no status set" {
    ...
    assert_mock_called_once curl -s -X POST \
        -H "Accept: application/vnd.github.v3+json" \
        "$GITHUB/repos/app-account/app-repo/statuses/main" \
        -d '{
            "state": "value",
            "context": "ci/cicd-repo",
...
@test "sets status of specified branch" {
    run status set test value
    assert_success
    assert_mock_called_once curl -s -X POST \
        -H "Accept: application/vnd.github.v3+json" \
        "$GITHUB/repos/app-account/app-repo/statuses/test" \
        -d '{
            "state": "value",
            "context": "ci/cicd-repo",
            "description": "Automated validation by cicd-repo CI."
...
@test "returns updated commit status" {
    set_mock_state query_post_response_state_result "value"
    run status set value
    assert_success
    assert_output "value"
